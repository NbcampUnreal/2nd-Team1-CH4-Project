...
bool UPlayMontageAction::PlayMontage()
{
    ...
    if (Duration <= 0.0f)
    {
        UE_LOG(LogTemp, Warning, TEXT("UPlayMontageAction::PlayMontage - Failed to play montage %s"), *Montage->GetName());
        return false;
    }
    ...
    UE_LOG(LogTemp, Log, TEXT("UPlayMontageAction::PlayMontage - Started playing montage %s"), *Montage->GetName());
    return true;
}
...
void UPlayMontageAction::OnMontageEnded(UAnimMontage* EndedMontage, bool bInterrupted)
{
    ...
    {
        UE_LOG(LogTemp, Log, TEXT("UPlayMontageAction::OnMontageEnded - Montage was interrupted, not changing state"));
        return;
    }
    ...
    {
        USmashStateSystem* StateSystem = OwnerState->OwnerStateSystem;
        if (StateSystem)
        {
            UE_LOG(LogTemp, Log, TEXT("UPlayMontageAction::OnMontageEnded - Changing state to %s"), *UEnum::GetValueAsString(NextState));
            StateSystem->TryChangeState(NextState);
        }
        else
        {
            UE_LOG(LogTemp, Warning, TEXT("UPlayMontageAction::OnMontageEnded - State system is null"));
        }
    }
}
